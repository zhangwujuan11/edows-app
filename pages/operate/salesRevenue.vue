    ...
    <view class="bottom" v-if="list && list.length > 0">
      ...
      <view class="content" v-for="(item, index) of list" :key="index">
        ...
        <view class="flex">
          ...
          <view class="input">{{ item.productSalesRevenue }}</view>
        </view>
      </view>
    </view>
    <view
...
<script>
import {
  outWarehouseProductAmountChart,
  currentMonthSales,
...
export default {
  ...
  methods: {
    ...
    getList(num, size) {
      ...
      outWarehouseProductAmountChart(params)
        .then((res) => {
          uni.hideLoading();
          if (res.code == 200) {
            this.list = size
    ...
    getEcharts() {
      currentMonthSales({ date: this.date }).then((res) => {
