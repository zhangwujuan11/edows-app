    ...
    <view class="content">
      ...
      color: #333333;">
        ...
        <view class="liconceng1">
          <text>门店名称:</text>
          <text>{{form.storeName}}</text>
        </view>
        <view class="liconceng1">
          <text>业 务 员：</text>
          <text>{{form.constructionWork}}</text>
        </view>
        <view class="liconceng1">
          <text>责任人：</text>
          <text>{{form.customerUser}}</text>
        </view>
        <view class="liconceng1">
          <text>责任单位：</text>
      ...
      <view class="licon" v-for="(item,index) in form.detailList" :key="index">
...
<script>
  ...
  export default {
    ...
    methods: {
      async getlist(id) {
        const res = await detaildamage(id)
        this.form = res.data
