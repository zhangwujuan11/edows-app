    ...
    <view class="main">
      ...
      <view style="margin-bottom: 30upx;" v-for="(item, index) in params.storeInWarehouseDetailBoList" :key="index">
        ...
        <view class="row" style="background: #f6f6f8">
          ...
          <text style="color: #b3b5c4;">
            {{num(index) |decimalFormat}}
          </text>
        </view>
      </view>
    </view>
    <view class="bottom">
      <view class="btn" @click="Return">确定</view>
    </view>
  ...
  export default {
    ...
    async onLoad() {
      ...
      if (data11.data.items) {
        ...
        this.params1.warehouseId = data11.data.items[0].warehouseId
        this.warehouseId = data11.data.items[0].warehouseId
        this.params.warehouseCode = data11.data.items[0].warehouseCode
      }
    },
    onBackPress(event) {
    ...
    methods: {
      ...
      async Return() {
        ...
        for (let i = 0; i <= this.Number; i++) {
          this.$refs.form[i].validate().then(async res66 => {
            const res1 = await addbenefit(this.params)
            if (res1.code == 200) {
              uni.showToast({
                title: "新增成功",
                icon: "none",
      ...
      join() {
        ...
        this.$set(this.params.storeInWarehouseDetailBoList[this.index], 'receivalQuantity', this.item
          .usableQuantity)
