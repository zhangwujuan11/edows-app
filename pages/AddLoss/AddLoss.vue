<template>
  <view style="padding-bottom: 148rpx">
    ...
    <view class="main">
      ...
      <view style="margin-bottom: 30upx;" v-for="(item, index) of params.detailList" :key="index">
        <view class="title">
    ...
    <view class="bottom">
      <view class="btn" @click="Return1" style="width: 335upx;">暂存</view>
  ...
  export default {
    ...
    methods: {
      ...
      async Return() {
        ...
        for (let i = 0; i <= this.Number; i++) {
          this.$refs.form[i].validate().then(async res66 => {
            const res1 = await adddamage(this.params)
            if (res1.code == 200) {
              uni.showToast({
                title: "新增成功",
                icon: "none",
      ...
      async Return1() {
        ...
        for (let i = 0; i <= this.Number; i++) {
          this.$refs.form[i].validate().then(async res66 => {
            const res9 = await adddamage(this.params)
            if (res9.code == 200) {
              uni.showToast({
                title: "暂存成功",
                icon: "none",
