  ...
  <view style="padding-bottom: 148rpx">
    <view class="top" style=" position: fixed;background-color: #fff; z-index: 999; ">
      ...
      <picker :value="params.status" :range="array2" mode="selector" @change="bindPickerChange2"
        style="width: 130upx; height: 72upx; background-color: #f1f1f1;border-radius: 30upx; position: relative; margin-right: 20upx;">
        <view v-if="array2[index2] != null" class="uni-input"
          style="height: 72upx;margin-left: 35upx; line-height: 72upx;font-size: 24upx;">
          {{ array2[index2] }}
          <uni-icons type="bottom" size="15"
            style="margin-left: 240upx; position: absolute;right: 15upx;color: #999999"></uni-icons>
        </view>
        <view v-else
          style="line-height: 72upx; margin-left: 30upx;font-weight: 400;font-size: 24upx;height: 72upx;color: #999999;">
          状态
          <uni-icons type="bottom" size="15"
            style="margin-left: 240upx; position: absolute;right: 15upx;color: #999999;"></uni-icons>
        </view>
      ...
      <view class="list" v-for="(item, index) in list" :key="index">
        <view>
          <view class="flex">
            ...
            <view class="status" v-if="item.status =='提交'" style="background-color: #fff9c1; color: #ffc262;">
              {{ item.status }}
            </view>
            <view class="status" v-else-if="item.status =='暂存'" style="background-color: #c7ffbb; color: #00d500;">
              {{ item.status }}
            </view>
            <view class="status" v-else style="background-color: #c2e8ff; color: #007DFF;">{{ item.status }}</view>
          </view>
          <view class="contain" @click="toList(item.outWarehouseId)">
            <view class="row">
...
width: 300upx;">{{ item.storeName }}</view>
          ...
          <view class="bottom">
            <view class="edit" @click="open1(item)" v-show="item.status != '出库' && item.status != '暂存'">审核通过</view>
            <view class="edit" @click="edit(item)" v-show="item.status != '出库'">修改</view>
            <view class="del" @click="open(item)" v-show="item.status != '出库'">删除</view>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      toList(e) {
        uni.navigateTo({
          url: `/pages/LossDetail/LossDetail?id=${e}`
        })
      },
      open(e) {
        this.currDel = e;
      ...
      add() {
        uni.navigateTo({
          url: "/pages/AddLoss/AddLoss"
        })
      },
      edit(e) {
        uni.navigateTo({
          url: "/pages/editLoss/editLoss?id=" + e.outWarehouseId
        })
      },
      getList(num, size) {
        let params1 = {
        ...
        switch (this.currIndex) {
          case 0:
            params1.storeName = this.searchValue;
            break;
          case 1:
      ...
      confirm() {
        ...
        deldamage(this.currDel.outWarehouseId).then((res) => {
          if (res.code == 200) {
            uni.hideLoading();
      ...
      confirm1() {
        ...
        Auditdamage(this.currDel).then((res) => {
          if (res.code == 200) {
            const number = this.list.length;
            this.getList(1, number);
            uni.hideLoading();
