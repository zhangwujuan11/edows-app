        ...
        <view class="header">
          ...
          <view class="search1">
            <view class="flex" @click="open(4)">
              <view class="sel" v-if="classification" style="font-size: 24upx;color: black; text-align: center;">
                {{ classification }}
              </view>
...
<script>
  ...
  export default {
    ...
    async onLoad() {
      const data11 = await getwarehouse(this.params3)
      if (data11.data.items) {
        this.outWarehouseName = data11.data.items[0].warehouseName
        this.params.outWarehouseName = this.outWarehouseName
        this.params.outWarehouseId = data11.data.items[0].warehouseId
        this.params.outWarehouseCode = data11.data.items[0].warehouseCode
        this.params.warehouseId = data11.data.items[0].warehouseId
        this.warehouseId = data11.data.items[0].warehouseId
      }
    },
    onBackPress(event) {
    ...
    methods: {
      ...
      async Return() {
        if (this.params.inWarehouseName == undefined) {
        ...
        } else if (this.params.agentUser == undefined) {
          return uni.showToast({
            title: "请选择经办人",
            icon: "none",
            duration: 1000,
          });
        } else if (this.params.orderType == undefined) {
          return uni.showToast({
            title: "请选择单据类型",
            icon: "none",
            duration: 1000,
          ...
          for (let i = 0; i <= this.Number; i++) {
            this.$refs.form[i].validate().then(async res66 => {
              const res1 = await addTransferStocks(this.params)
              if (res1.code == 200) {
                uni.showToast({
                  title: "新增成功",
                  icon: "none",
