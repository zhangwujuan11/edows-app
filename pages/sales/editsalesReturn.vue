<template>
  <view style="padding-bottom: 148rpx">
    ...
    <view class="bottom" style="display: flex;">
      <view class="btn" style="height: 88upx; width: 335upx;" @click="Return1" v-show='params.status==0'>暂存</view>
...
<script>
  ...
  export default {
    ...
    async onLoad(e) {
      const res2 = await detailoutWarehouseReturn(e.id)
      this.params = res2.data
      this.customer = this.params.customer
      this.storeName = this.params.storeName
      this.constructionWork = this.params.constructionWork
      if (this.params.delivery == 0) {
        this.delivery = '自提'
      } else if (this.params.delivery == 1) {
        this.delivery = '送货'
      } else if (this.params.delivery == 2) {
        this.delivery = '物流代收'
      } else {
        this.delivery = '物流不代收'
      }

      if (this.params.paymentTerms == 1) {
        this.paymentTerms = '现金付款'
      } else if (this.params.paymentTerms == 2) {
        this.paymentTerms = '银行转账'
      } else if (this.params.paymentTerms == 3) {
        this.paymentTerms = '微信'
      } else {
        this.paymentTerms = '挂账'
      }
    },
    filters: {
      decimalFormat(value) {
    ...
    methods: {
      async open(e) {
        ...
        switch (e) {
          case 0:
            this.title = '请选择往来客户'
            const data = await getcustomer(this.params1)
            if (data.data.items) {
      ...
      async scrolltolowerFun() {
        ...
        this.params1.customerId = this.customerId
        ...
        if (this.params.paymentTerms == undefined) {
          ...
          for (let i = 0; i <= this.Number; i++) {
            this.$refs.form[i].validate().then(async res66 => {
              const res1 = await editoutWarehouseReturn(this.params)
              if (res1.code == 200) {
                uni.showToast({
                  title: "修改成功",
                  icon: "none",
      ...
      async Return1() {
        ...
        if (this.params.paymentTerms == undefined) {
          ...
          for (let i = 0; i <= this.Number; i++) {
            this.$refs.form[i].validate().then(async res66 => {
              const res4 = await editoutWarehouseReturn(this.params)
              if (res4.code == 200) {
                uni.showToast({
                  title: "暂存成功",
                  icon: "none",
