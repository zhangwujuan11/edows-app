<template>
  <view style="padding-bottom: 148rpx">
      ...
      <view class="row-select">
        ...
        <view class="flex">
          <uni-datetime-picker type="date" :clear-icon="true" v-model="params.enteringTheFactoryDate"
            @change="maskClick">
            <view class="input" v-if="params.enteringTheFactoryDate">
              <uni-dateformat :date="params.enteringTheFactoryDate" format="yyyy/MM/dd"></uni-dateformat>
            </view>
            <view v-else class="sel">请选择</view>
          </uni-datetime-picker>
          <image class="expand" src="/static/mine/to.png"></image>
      ...
      <view class="row-select">
        ...
        <view class="flex">
          <uni-datetime-picker type="date" :clear-icon="true" v-model="params.expirationDate" @change="maskClick1">
            <view class="input" v-if="params.expirationDate"><uni-dateformat :date="params.expirationDate"
    ...
    <view class="bottom" style="display: flex;">
      <view class="btn" style="height: 88upx; width: 335upx;" v-show="status==0" @click="add1">暂存</view>
...
<script>
  ...
  export default {
    ...
    async onLoad(e) {
      if (e.id != null) {
        const res3 = await detailoutWarehouseForeground(e.id)
        console.log(res3)
        ...
        this.warehouseId = res3.data.storeId
        this.status = res3.data.status
        ...
        if (this.params.paymentTerms == undefined) {
          ...
          for (let i = 0; i <= this.Number; i++) {
            this.$refs.form[i].validate().then(async res66 => {
              const res1 = await editoutWarehouseForeground(this.params)
              if (res1.code == 200) {
                uni.showToast({
                  title: "修改成功",
                  icon: "none",
