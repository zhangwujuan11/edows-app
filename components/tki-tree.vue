<template xlang="wxml">
	<view class="tki-tree">
		<view class="tki-tree-mask" :class="showTree? 'show' : 'hide'" @tap="_cancel"></view>
		<view class="tki-tree-cnt"  :class="showTree? 'show' : 'hide'">
			<view class="tki-tree-bar">
				<view class="tki-tree-bar-cancel" :style="{'color':cancelColor}" hover-class="hover-c" @tap="_cancel">取消</view>
				<view class="tki-tree-bar-title" :style="{'color':titleColor}">请选择产品参数</view>
			...
			_confirm() {
				...
				this.treeList.forEach((v, i) => {
					if (this.treeList[i].checked) {
						obj = {}
						obj.paramId = this.treeList[i].source.id
						obj.paramName = this.treeList[i].source.label
